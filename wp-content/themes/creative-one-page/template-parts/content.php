...
<article class="row page-box p-4 mb-4">
  <div class="post-thumb col-lg-5 col-md-5">
    <?php if(has_post_thumbnail() && get_theme_mod( 'creative_one_page_show_featured_image_post',true) != '') { ?>
      <div class="box-img">
        <?php the_post_thumbnail(); ?>
      </div>
    <?php } ?>
  </div>
  <div class="new-text <?php if(has_post_thumbnail() && get_theme_mod( 'creative_one_page_show_featured_image_post',true) != '') { ?>col-lg-7 col-md-7<?php } else { ?>col-lg-12 col-md-12 <?php } ?>">
    <?php if( get_theme_mod( 'creative_one_page_date_hide',true) != '' || get_theme_mod( 'creative_one_page_author_hide',true) != '' || get_theme_mod( 'creative_one_page_comment_hide',true) != '' || get_theme_mod( 'creative_one_page_time_hide',true) != '') { ?>
      <div class="metabox">
        <?php if( get_theme_mod( 'creative_one_page_date_hide',true) != '') { ?>
          <span class="entry-date me-1"><i class="<?php echo esc_attr(get_theme_mod('creative_one_page_date_icon','fa fa-calendar')); ?> me-2"></i><a href="<?php echo esc_url( get_day_link( $archive_year, $archive_month, $archive_day)); ?>"><?php echo esc_html( get_the_date() ); ?><span class="screen-reader-text"><?php echo esc_html( get_the_date() ); ?></span></a></span><span class="meta-seperator"><?php echo esc_html( get_theme_mod('creative_one_page_metabox_separator_blog_post','|') ); ?></span>
        <?php } ?>
        <?php if( get_theme_mod( 'creative_one_page_author_hide',true) != '') { ?>
          <span class="entry-author mx-1"><i class="<?php echo esc_attr(get_theme_mod('creative_one_page_author_icon','fa fa-user')); ?> me-2"></i><a href="<?php echo esc_url( get_author_posts_url( get_the_author_meta( 'ID' )) ); ?>"><?php the_author(); ?><span class="screen-reader-text"><?php the_author(); ?></span></a></span><span class="meta-seperator"><?php echo esc_html( get_theme_mod('creative_one_page_metabox_separator_blog_post','|') ); ?></span>
        <?php } ?>
        <?php if( get_theme_mod( 'creative_one_page_comment_hide',true) != '') { ?>
          <span class="entry-comments mx-1"><i class="<?php echo esc_attr(get_theme_mod('creative_one_page_comment_icon','fas fa-comments')); ?> me-2"></i><?php comments_number( __('0 Comments','creative-one-page'), __('0 Comments','creative-one-page'), __('% Comments','creative-one-page') ); ?></span><span class="meta-seperator"><?php echo esc_html( get_theme_mod('creative_one_page_metabox_separator_blog_post','|') ); ?></span>
        <?php } ?>
        <?php if( get_theme_mod( 'creative_one_page_time_hide',true) != '') { ?>
          <span class="entry-time mx-1"><i class="<?php echo esc_attr(get_theme_mod('creative_one_page_time_icon','fas fa-clock')); ?> me-1"></i><?php echo esc_html( get_the_time() ); ?></span>
        <?php }?>
      </div>
    <?php }?>
